...
export const POST = async (NextRequest, NextResponse) => {
    ...
    try {
        ...
        if (!user_id || !address) {
            return new Response("User ID and address are required", { status: 400 });
        }
        ...
        await user.save();
        return new Response("Address updated successfully", { status: 200 });
    } catch (error) {
...
export const GET = async (NextRequest, NextResponse) => {
    ...
    try {
        const { user_id } = NextRequest.headers.get('user_id');
